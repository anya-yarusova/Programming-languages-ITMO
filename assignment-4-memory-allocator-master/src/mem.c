...
static struct region region_init(void* restrict addr, block_size region_size, bool extends) {
...
static struct region alloc_region(void const * addr, size_t query) {
  const block_size region_size = {
    .bytes = region_actual_size(size_from_capacity((block_capacity) {query}).bytes)
  };
  void* new_addr = map_pages(addr, region_size.bytes, MAP_FIXED_NOREPLACE);
  if (new_addr == MAP_FAILED) {
    new_addr = map_pages(addr, region_size.bytes, 0);
    if (new_addr == MAP_FAILED) {
      return REGION_INVALID;
    }
  }
  block_init(new_addr, region_size, NULL);
  return region_init(new_addr, region_size, new_addr == addr);
}
...
static bool split_if_too_big(struct block_header* block, size_t query) {
  ...
  void* restrict const new_block_addr = block_after(block);
...
static struct block_search_result find_good_or_last(struct block_header* restrict block, size_t size) {
  while (block != NULL) {
    ...
    if (block_is_big_enough(size, block) && block->is_free) {
...
static struct block_header* memalloc(size_t query, struct block_header* heap_start) {
  ...
  if (result.type == BSR_REACHED_END_NOT_FOUND) {
    struct block_header* const new_block = grow_heap(result.block, query);
    if (new_block == NULL) {
      return NULL;
    } else {
